...
void Collage::drawByCoordinatesSerial(PNG * result)
{
    ...
    for(int i = 0; i < width; ++i)
    {
        for(int j = 0; j < height; ++j)
        {
			...
			for(int layer = 0; layer < layers.size(); layer++)
			{
				int offsetX = coords[layer].x;
				int offsetY = coords[layer].y;
				if((i < (offsetX + layers[layer].width))&&(i >= offsetX)&&(j < (offsetY + layers[layer].height))&&(j >= offsetY))
					*(*result)(i,j) = *(layers[layer](i-offsetX, j-offsetY));
			}
