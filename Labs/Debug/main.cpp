...
int in;
	...
	original = new PNG;					// made pointer point to something
	...
	// Create out.png
	PNG * output;
	output = setupOutput(width, height); // output = output of helper function		
	...
	{
		for (int x = 1; x < width; x++)					//infinite loop before
		{
			...
			RGBAPixel * currOutPixel = (*output)(x,y);    // Wasn't changing the output before
			if (diff > 100)
			{
	...
	output->writeToFile("out.png");  			// image width/height problem
	
	// Clean up memory
	myPixel = NULL;
	delete myPixel;								// poop seg fault
	delete output;
	delete original;
	return 0;
}
