...
class KDTree
{
    ...
    private:
         ...
         */
		int partition(vector< Point<Dim> > & point_list, int left, int right, int pivotIndex, int curDim);
		Point<Dim> select(vector< Point<Dim> > & point_list, int left, int right, int k, int curDim);
		void BuildTree(vector< Point<Dim> > & pointsList, int left, int right, int curDim);
		Point<Dim> NearestNeighborHelper(const Point<Dim> & query, int left, int right, int curDim) const;
};
